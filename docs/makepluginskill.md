...
- 需要自动撤回用户输入时使用 `ctx.listen(timeout, retract_time)`，两个参数均为秒；撤回的是本次 `listen` 收到的用户消息，不是触发插件的原消息。
- 超时要返回可理解的提示，不要让流程悬空。
- 管理类操作建议使用“序号选择 + q 退出 + y 确认”的交互模式。

### 群管理
- 使用 `ctx.deleteMessage()` / `ctx.delete_message()` 撤回当前消息。
- 使用 `ctx.mute(...)` 禁言群成员；`user_id` 为空时表示请求群全体禁言。适配器未支持时应按返回的 `False` 继续流程。
