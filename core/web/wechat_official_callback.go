...
const wechatOfficialCallbackPrefix = "/api/open/adapters/wechat-official/"

func (s *Server) handleWechatOfficialAdapterCallback(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet && r.Method != http.MethodPost {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	adapterID, relativePath, ok := parseWechatOfficialCallbackPath(r.URL.Path)
	if !ok {
	...
	if adp.GetPlatform() != "wechat_official" {
		http.NotFound(w, r)
		return
	}
...
func parseWechatOfficialCallbackPath(path string) (int64, string, bool) {
	rest := strings.TrimPrefix(path, wechatOfficialCallbackPrefix)
	if rest == path || rest == "" {
		return 0, "", false
	}
